...
2. **Given** topic has been validated as relevant, **When** validation response streams in, **Then** user sees 8-20 historical figures appear progressively with avatar, name, handle (id), tagline, and bio (streaming via Server-Sent Events for better UX)
3. **Given** user suggested panelist names during topic entry, **When** validation response streams in, **Then** AI evaluates suggested names and includes them in the panelist list if they have known, documented positions on the topic
...
- **FR-004**: System MUST request and display 8-20 panelist suggestions with complete profiles (name, avatar URL, handle (id), tagline, biography) using progressive streaming via Server-Sent Events (SSE)
...
- **FR-004b**: System MUST stream panelists incrementally as they are generated by Claude API, displaying each panelist immediately upon completion rather than waiting for all panelists
